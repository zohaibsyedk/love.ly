...
struct PromptView: View {
    ...
    var body: some View {
        VStack {
            ...
            Text("Compliments selected for \(prompt): \(productSetupManager.complimentsStore[prompt]?.count ?? 0)")
            List {
                ForEach(productSetupManager.complimentsStore[prompt] ?? [], id: \.self) { item in
                    Text(item)
                }
                .onDelete {indexSet in
                    productSetupManager.complimentsStore[prompt]?.remove(atOffsets: indexSet)
                    productSetupManager.counts -= 1
